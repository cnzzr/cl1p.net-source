...
{
    private static Logger log = Logger.getLogger(TestDiff.class);
    ...
    protected void runDiff(Diff diff, Difference[] expected)
    {
        log("expected", expected);

        // Integer[] lcs = diff.getLongestCommonSubsequences();
        // tr.Ace.log("lcs", lcs);
        ...
        for (int ei = 0; ei < expected.length; ++ei) {
            Difference d   = (Difference)diffOut.get(ei);
            Difference exp = expected[ei];
            assertEquals("expected[" + ei +"]", exp, d);
        }
    }

    private static void log(String a, List b){
        for(Object o : b)
            log.debug(a + " " + o);
    }

    private static void log(String a, Object[] oa){
        for(Object o : oa)
            log.debug(a + " " + o);
    }
